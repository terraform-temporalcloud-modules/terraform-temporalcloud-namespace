...
| `disabled` | `create_namespace = false` produces no resources and every output falls back via `try()` |
| `minimal` | The module works with only `name`, `regions` and `retention_days` |
